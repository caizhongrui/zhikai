...
export default defineConfig(({ mode }) => {
	let outDir = "../out/webview-ui"

	// 使用扩展根目录的 package.json
	const getPkg = () => {
		try {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("天和 AI 助手"),
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@roo-code/types": resolve(__dirname, "../src/packages/types"),
				"@roo-code/telemetry": resolve(__dirname, "../src/packages/telemetry"),
				"@roo-code/cloud": resolve(__dirname, "../src/packages/cloud"),
			},
		},
		build: {
			outDir,

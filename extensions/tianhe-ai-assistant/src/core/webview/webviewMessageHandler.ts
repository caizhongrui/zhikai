...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		// 知识库相关消息处理
		case "fetchKnowledgeBases": {
			try {
				// 获取配置
				const config = vscode.workspace.getConfiguration("zhikai.auth")
				const apiUrl = config.get<string>("apiUrl")
				const username = config.get<string>("username")
				const password = config.get<string>("password")

				if (!apiUrl || !username || !password) {
					provider.log("Knowledge base fetch failed: missing configuration")
					await provider.postMessageToWebview({
				...
				const response = await axios.get(
					`${baseUrl}/knowledge/knowledgeApplication/listByUser?applicationStatus=0`,
					{
						headers: {
				...
				if (response.data && response.data.data) {
					const knowledgeBases = response.data.data.map((item: any) => ({
						id: item.id,
				...
				provider.log(`Error fetching knowledge bases: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`)
				await provider.postMessageToWebview({
					type: "knowledgeBaseList",
					knowledgeBases: [],
				})
		...
		case "selectKnowledgeBase": {
			try {
				const selectedId = message.text || null
				await updateGlobalState("selectedKnowledgeBaseId", selectedId)
				await provider.postMessageToWebview({
					type: "knowledgeBaseSelected",
					selectedKnowledgeBaseId: selectedId,
				})
			} catch (error) {
				provider.log(`Error selecting knowledge base: ${JSON.stringify(error, Object.getOwnPropertyNames(error), 2)}`)
			}
			break
		}
	}

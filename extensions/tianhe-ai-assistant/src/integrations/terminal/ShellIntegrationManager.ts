...
export class ShellIntegrationManager {
	...
	public static zshInitTmpDir(env: Record<string, string>): string {
		...
		vscode.workspace.fs
			...
			.then(() => {
				...
				// 如果 shell integration 路径为空，跳过
				if (!shellIntegrationPath) {
					console.warn(`[TerminalRegistry] Skipping shell integration (path not available)`)
					return tmpDir
				}
	...
	private static getShellIntegrationPath(shell: "bash" | "pwsh" | "zsh" | "fish"): string {
		...
		if (!appRoot) {
			console.warn(`[ShellIntegrationManager] vscode.env.appRoot is undefined, using fallback`)
			// Fallback: 返回空字符串，调用者需要处理
			return ""
		}

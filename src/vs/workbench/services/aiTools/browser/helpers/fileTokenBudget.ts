...
// Mock Anthropic namespace for type compatibility
namespace Anthropic {
...
// Local implementation of countTokens
async function countTokens(content: Anthropic.Messages.ContentBlockParam[]): Promise<number> {
...
// Local implementation of countFileLinesAndTokens
async function countFileLinesAndTokens(
	filePath: string,
	options: { budgetTokens: number; chunkLines: number }
): Promise<{ tokenEstimate: number; complete: boolean; lines: number }> {
	try {
		const content = await fs.readFile(filePath, 'utf-8');
		const lines = content.split('\n');
		const tokenEstimate = Math.ceil(content.length / 4);
		const complete = tokenEstimate <= options.budgetTokens;
		return { tokenEstimate, complete, lines: lines.length };
	} catch (error) {
		throw error;
	}
}

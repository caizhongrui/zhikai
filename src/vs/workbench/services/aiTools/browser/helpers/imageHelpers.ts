...
// Local implementation of i18n function
function t(key: string, params?: any): string {
	// Simple translation fallback
	if (key === "tools:readFile.imageTooLarge") {
		return `Image too large (${params?.size}). Maximum size is ${params?.max}MB.`;
	}
	if (key === "tools:readFile.imageWithSize") {
		return `Image (${params?.size}KB)`;
	}
	return key;
}

// Local implementation of prettyBytes

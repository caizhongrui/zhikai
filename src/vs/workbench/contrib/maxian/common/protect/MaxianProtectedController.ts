...
// Copied from Kilocode src/core/protect/RooProtectedController.ts
// Adapted for tianhe-zhikai-ide: renamed to MaxianProtectedController, updated protected patterns
...
/**
 * Controls write access to Maxian configuration files by enforcing protection patterns.
 * Prevents auto-approved modifications to sensitive Maxian configuration files.
 */
export class MaxianProtectedController {
	private cwd: string;
	private ignoreInstance: Ignore;

	// Predefined list of protected Maxian configuration patterns
	private static readonly PROTECTED_PATTERNS = [
		...
		'.maxian/**',
		'.maxianprotected',
		// Legacy Kilocode/Roo patterns for compatibility
		'.kilocodeignore',
		'.kilocodemodes',
		'.kilocoderules',
		'.kilocode/**',
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(MaxianProtectedController.PROTECTED_PATTERNS);
	}
	...
	getProtectionMessage(): string {
		return 'This is a Maxian configuration file and requires approval for modifications';
	}
	...
	getInstructions(): string {
		const patterns = MaxianProtectedController.PROTECTED_PATTERNS.join(', ');
		return `# Protected Files\n\n(The following Maxian configuration file patterns are write-protected and always require approval for modifications, regardless of autoapproval settings. When using list_files, you'll notice a ${SHIELD_SYMBOL} next to files that are write-protected.)\n\nProtected patterns: ${patterns}`;
	}
	...
	static getProtectedPatterns(): readonly string[] {
		return MaxianProtectedController.PROTECTED_PATTERNS;
	}
}
